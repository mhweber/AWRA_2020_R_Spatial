...
Typically working with vector GIS data we work with ESRI shapefiles or geodatabases - here we have an example of how one would read in a shapefie using `sf`:
...
# want to get rid of scientific notation in your plot?
```{r shapefile_read2, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
options(scipen=3)
plot(citylims$geometry, axes=T, main='Oregon City Limits')
```
...
```{r osmdata , message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
library(osmdata)
library(mapview)
...
We often have flat files, locally on our machine or acceessed elsewhere, that have coordinate inforamtion which we would like to make spatial:
