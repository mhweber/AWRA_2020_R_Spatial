...
`penguins` is a tibble and was created as a new alternative to the `iris` data set that has been used extensively for beginning tutorials on learning R. `penguins` as you can see is a [tibble](https://tibble.tidyverse.org/), which is a new `tidyverse` spin on `data frames`. Data frames consist of rows of observations  on columns of values for variables of interest - they are one of the fundamental and most important data structures in R. `tibbles` make a few improvements / changes to `data frames` such as:
...
>>>>>>> 3aeaae3b947da3e86b98a372cdb7eb1342a5cec9
